  ...
  $.fn.smartZoom = function(method) {
    ...
    var publicMethods = {
	    ...
	    init : function(options) {
		    ...
		    var containerDiv = $('<div id="'+zoomContainerId+'" class="'+settings.containerClass+'" style="margin-left: -0px;"></div>');
		    targetElement.before(containerDiv);
		    targetElement.remove();
		    containerDiv = $('#'+zoomContainerId);
		    containerDiv.css({'overflow':'visible'});
		    ...
		    targetElement.data('smartZoomData', {
               ...
               mouseWheelDeltaFactor:0.5, // enable to slow down the zoom via mouse wheel
    ...
    function getValidTargetElementPosition(xPosition, yPosition, width, height){
        ...
        // 设置方形区域的边界（50x50 像素扩展到每个方向）
        var squareHalfSize = -50; // 方形区域每边延伸的半径

        // 确定允许的最大边界
        var maxLeftBoundary = viewportCenterX - squareHalfSize;
        var maxTopBoundary = viewportCenterY - squareHalfSize;
        var maxRightBoundary = viewportCenterX + squareHalfSize;
        var maxBottomBoundary = viewportCenterY + squareHalfSize;

        // 调整位置以保持在方形区域内
        var newMarginLeft = Math.min(Math.max(xPosition, maxLeftBoundary - width), maxRightBoundary);
        var newMarginTop = Math.min(Math.max(yPosition, maxTopBoundary - height), maxBottomBoundary);

        return new Point(newMarginLeft, newMarginTop);
    }
