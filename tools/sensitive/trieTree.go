...
// Trie 结构，表示一个前缀树，用于存储和检索敏感词。
type Trie struct {
	Root *Node // 树的根节点
}

// Node 结构，表示 Trie 树中的一个节点。
type Node struct {
	isRootNode bool           // 标记是否为根节点
	isPathEnd  bool           // 标记这个节点是否是某个敏感词的结束
	Character  rune           // 节点存储的字符
	Children   map[rune]*Node // 存储子节点的映射
}

// NewTrie 创建并返回一个新的 Trie 树实例。
func NewTrie() *Trie {
	return &Trie{
		Root: NewRootNode(0),
	}
...
// Add 方法用于向 Trie 树中添加一组敏感词。（导出的函数）
...
// add 方法用于向 Trie 树中添加一个敏感词。（非导出的函数）
