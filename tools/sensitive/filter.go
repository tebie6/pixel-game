...
// 项目来源：https://github.com/importcjj/sensitive/tree/master
import (
	"bufio"
	"fmt"
	"io"
...
// InitSensitive 初始化敏感词过滤器
func InitSensitive(filepath string) {
	SensitiveFilter = New()
	if err := SensitiveFilter.loadWordDict(filepath); err != nil {
		panic(fmt.Errorf("the file %s that for sensitive filter is not exist", filepath))
	}
}
